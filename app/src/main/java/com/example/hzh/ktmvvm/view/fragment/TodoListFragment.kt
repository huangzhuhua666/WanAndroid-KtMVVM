...
class TodoListFragment : WanFragment<FragmentTodoListBinding, TodoVM>() {
    ...
    override fun initListener() {
        LiveEventBus.get("save_todo").observe(viewLifecycleOwner, Observer {
            //添加、编辑需要刷新列表
            if (status == 0) mViewModel?.getInitData(false)
        })
        LiveEventBus.get("todo_status", Int::class.java).observe(viewLifecycleOwner, Observer {
            if (status == it) mViewModel?.getInitData(false) // 刷新列表
        })
        LiveEventBus.get("todo_delete", Todo::class.java).observe(viewLifecycleOwner, Observer {
